...
import { convertSecondsToMinutes, doFuncAtGivenTimeByEstimate, getNumFromComponent, moveDown, swipeDown, waitRandomTime } from "../common/utils";
import { MIN_RUN_THRESHOLD, NAME_VEDIO_KUAISHOU_LITE, NORMAL_WAIT_TIME, PACKAGE_VEDIO_KUAISHOU_LITE } from "../global";
...
export class KuaiShouLite extends Base{
    ...
    weight(): void {
        ...
        const coin = scrollTo("我的金币")
        if(coin){
            const component = searchByOcrRecognize("[0-9]+", {
                bounds: {
                    top: coin.bounds().bottom,
                    bottom: coin.bounds().bottom + 300,
                    right: device.width/2
                }
            })
            if(component){
    ...
    openTreasure(): void {
        this.goTo(this.tab, 2)
        this.watchAdsForCoin("日常福利")
        if(fixedClick("开宝箱得金币|点就领 [0-9]+金币")) {
