...
function searchByLeftRange(button, options, times) {
  ...
  if (typeof options.leftRange === "string") {
    var tmp = searchByUiSelect(textMatches(options.leftRange)) || searchByUiSelect(descMatches(options.leftRange));
    ...
    if (tmp == null) {
      ...
      if (typeof button === "string") {
        return searchByUiSelect(textMatches(button).boundsInside(0, top, device.width, bottom), options) || searchByUiSelect(descMatches(button).boundsInside(0, top, device.width, bottom));
      } else {
        return searchByUiSelect(button.boundsInside(0, top, device.width, bottom), options);
      }
    }
  }
}
function searchByOcrRecognize(str, options) {
  ...
  if (list.length > 0) {
    ...
    if (options === null || options === void 0 ? void 0 : options.position) {
      logger_Record.debug("position: ".concat(options.position));
      ...
      if (typeof options.position === "number") {
        ...
        for (var _b = 0, list_1 = list; _b < list_1.length; _b++) {
          var item = list_1[_b];
          var bounds_1 = item.bounds;
          var controlX = (bounds_1.left + bounds_1.right) / 2;
          var controlY = (bounds_1.top + bounds_1.bottom) / 2;
          var distance = calculateDistance(options.position, {
            x: controlX,
            y: controlY
          });
...
function searchByUiSelect(component, options) {
  ...
  if ((options === null || options === void 0 ? void 0 : options.waitFor) || (options === null || options === void 0 ? void 0 : options.position)) {
    if (options.waitFor) {
      ...
      if (list.nonEmpty()) {
        if (typeof options.position === "number") {
          ...
          for (var i = 0; i < list.length; i++) {
            var control = list[i];
            var bounds_2 = control.bounds();
            var controlX = (bounds_2.left + bounds_2.right) / 2;
            var controlY = (bounds_2.top + bounds_2.bottom) / 2;
            var distance = calculateDistance(options.position, {
              x: controlX,
              y: controlY
            });
            ...
            if (distance < minDistance) {
              logger_Record.debug("select item: x=".concat(controlX, ",y=").concat(controlY));
              minDistance = distance;
              result = control;
            }
          }
    ...
    result = component.findOnce();
  }

  if (result && (options === null || options === void 0 ? void 0 : options.bounds)) {
    var bounds_3 = result.bounds();

    if (!myBoundsContains(options.bounds, bounds_3)) {
      return null;
    }
  }
...
function findAndClick(component, options, times) {
  ...
  if (bounds) {
    if (options === null || options === void 0 ? void 0 : options.selected) {
      ...
      var index = findLargestIndexes(list, 2);
...
function utils_close(times) {
  if (!closeByImageMatching()) {
    switch (times % 2) {
      case 0:
        if (!findAndClick(className("android.widget.ImageView").boundsInside(945, 0, device.width, 309))) {
          back();
          waitRandomTime(4);
        }
      ...
      case 1:
        if (!findAndClick(className("android.widget.ImageView").boundsInside(0, 0, 132, 273))) {
          back();
          waitRandomTime(4);
        }
...
var Base = function () {
  ...
  Base.prototype.watchAdsForCoin = function (backSign) {
    ...
    while (++cycleCounts < MAX_CYCLES_COUNTS && goneClick("看.*(视频|内容).*(得|领|赚).*[0-9]+金币")) {
  ...
  return function (target, key, descriptor) {
    ...
    descriptor.value = function () {
      ...
      try {
        ...
        logger_Record.debug("".concat(error));
        ...
        while (retries < MAX_RETRY_COUNTS) {
          ...
          try {
            ...
            logger_Record.error("".concat(error));
            sendErrorMessage();
            retries++;
          }
        }
  ...
  descriptor.value = function () {
    ...
    try {
      ...
      if (isCurrentAppBanned(e)) {
        ...
        logger_Record.error("\u5F53\u524Dapp\u53D1\u751F\u5F02\u5E38: ".concat(e));
        sendErrorMessage();
      }
    }
...
var DeJian = function (_super) {
  ...
  DeJian.prototype.readBook = function (totalTime) {
    ...
    if (tmp != null && findAndClick("推荐", {
      ...
      if (findAndClick(id("com.zhangyue.iReader.bookStore:id/iv_book"), {
        fixed: true
      })) {
        this.read(totalTime);
      }
...
var KuaiShouFree = function (_super) {
  ...
  KuaiShouFree.prototype.lowEff = function (time) {
    ...
    doFuncAtGivenTime(time, 30 * 60, function (perTime) {
      _this.readBook(perTime);
  ...
  KuaiShouFree.prototype.weight = function () {
    ...
    this.store(Base_BaseKey.Weight, cCoin - this.coin);
  };
...
var MarvelFree = function (_super) {
  ...
  MarvelFree.prototype.readBook = function (totalTime) {
    ...
    if (selectedClick("推荐")) {
      if (findAndClick(id(PACKAGE_READ_MARVEL_FREE + ":id/tv_book_name"), {
        fixed: true,
        position: random(0, 7)
      })) {
        this.read(totalTime);
...
var AbstractTomato = function (_super) {
  ...
  AbstractTomato.prototype.sign = function () {
    try {
      ...
      if (isCurrentAppBanned(error)) {
        logger_Record.warn("".concat(this.appName, "\u8D26\u53F7\u5F02\u5E38, \u4F46\u4E0D\u5F71\u54CD\u540E\u7EED\u6536\u76CA"));
...
var RedFruits = function (_super) {
  ...
  function RedFruits() {
    ...
    _this.highEffEstimatedTime = _this.fetch(Base_BaseKey.highEffEstimatedTime, global_BASE_ASSIMT_TIME);
    _this.medEffEstimatedTime = _this.fetch(Base_BaseKey.medEffEstimatedTime, 40 * 60);
    _this.lowEffEstimatedTime = 0;
    _this.lowEffAssmitCount = 2;
    return _this;
  }
  ...
  RedFruits.prototype.weight = function () {
    ...
    if (tmp != null) {
      logger_Record.log("".concat(this.constructor.name, ":").concat(parseInt(tmp.text())));
      this.store(Base_BaseKey.Weight, parseInt(tmp.text()));
    }
  };
  ...
  RedFruits_decorate([measureExecutionTime], RedFruits.prototype, "medEff", null);
...
var SpeedFree = function (_super) {
  ...
  function SpeedFree() {
    ...
    _this.highEffEstimatedTime = _this.fetch(Base_BaseKey.highEffEstimatedTime, global_BASE_ASSIMT_TIME);
    _this.medEffEstimatedTime = _this.fetch(Base_BaseKey.medEffEstimatedTime, 20 * 60);
    _this.lowEffEstimatedTime = 0;
    return _this;
  }
  ...
  SpeedFree_decorate([measureExecutionTime], SpeedFree.prototype, "medEff", null);
...
var TomatoLite = function (_super) {
  ...
  TomatoLite.prototype.highEff = function () {
    ...
    randomExecute([function () {
      ...
      if (_this.situation) _this.mealSupp();
    }, function () {
      _this.winGoldCoin();
    }, function () {
      _this.listenBook();
...
var NAME_READ_RED_FRUITS = "番茄免费短剧";
...
function init() {
  ...
  threads.start(function () {
    var pattern = ".?立即开始.?|.?允许.?";
    var tmp = className("android.widget.Button").textMatches(pattern).findOne(5000);
